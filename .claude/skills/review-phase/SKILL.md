...
- **The row** is the one whose *File* column names the plan's file, or else the one whose *Plan* column names the plan: the table has one row per plan, started or not. When no row names it, the plan was never listed: add its row, in the plan's place in the §12.2 order.
