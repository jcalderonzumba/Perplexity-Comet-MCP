...
7. **Keep the records the task names.** A task's acceptance often names `README.md`, `CHANGELOG.md` or the AGENTS.md Commands table: update them in the same commit as the code, written as the description of a finished project. A follow-up you discover goes where AGENTS.md workflow step 10 places it, in the same commit, and your report lists it.
