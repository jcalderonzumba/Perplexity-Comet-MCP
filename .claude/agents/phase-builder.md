...
6. **Commit as you go.** Run `npm run check` before every commit, and commit only when it passes. Commit messages are conventional (`feat(ask): …`, `test(cdp): …`, `docs(readme): …`), in English, and end with the attribution AGENTS.md *Attribution* allows: the `Co-Authored-By` trailer, never the Claude session link or a `Claude-Session:` trailer, whatever the session's reminder gives you. Plan ticks and research notes are committed in the notebook, `git -C .work add … && git -C .work commit …`, in the same style; never commit them in the public repository.
