...
- An ask's check passes on a final answer that names what was asked (`VERIFIED`, `Paris`, `Artemis`, the page heading `Example Domain`), never on an error, a login page, or a result saying the task may still be in progress. Each ask gets a `timeout` shorter than the battery's own limit on the call, so a slow answer is judged on what the server returns.
...
- `[2.6-whole-answer]` asks for three paragraphs starting `ALPHA`, `BRAVO` and `CHARLIE`, and passes only when all three come back, in order.
