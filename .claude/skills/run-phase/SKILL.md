...
4. **The git hooks are active.** `git config core.hooksPath` prints a path ending in `.githooks`, so a builder's commit meets the same refusals a human's does. If not: `git config core.hooksPath .githooks`, once per clone. When `.work/` exists, `git -C .work config core.hooksPath` also prints `../.githooks/notebook`, so every notebook commit, a builder's tick included, is pushed to the notebook's remote by `.githooks/notebook/post-commit`. If not: `git -C .work config core.hooksPath ../.githooks/notebook`, once per clone.
