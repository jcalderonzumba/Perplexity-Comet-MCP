...
3. **Process rules.**
   ...
   - Commit messages in English, ending with the attribution AGENTS.md *Attribution* allows; no commit message, PR draft, code or doc in the diff carries the Claude session link (`https://claude.ai/code/session_…` or a `Claude-Session:` trailer); code taken from upstream carries a `Co-authored-by` trailer and links the upstream PR (AGENTS.md workflow step 11).
