...
3. **Process rules.**
   ...
   - A change to the spec in the work range is accompanied by a decision-log row in its §13 in the same `.work` commit. There are two exceptions, which track work, change no design and need no row: adding or removing an item in the follow-up list in spec §12.3 (AGENTS.md workflow step 10), and bringing a plan's row of the spec's §12 status table up to date, as `/review-phase` §9 does (spec D20).
   ...
   - Commit messages in English, ending with the attribution AGENTS.md *Attribution* allows; no commit message, PR draft, code or doc in the diff, and no notebook commit in the work range, carries the Claude session link (`https://claude.ai/code/session_…` or a `Claude-Session:` trailer); code taken from upstream carries a `Co-authored-by` trailer and links the upstream PR (AGENTS.md workflow step 11).
4. **Language.** Code, comments, docs, commit messages and tool output are in English.
5. **Tests.** New behaviour has a test. `*.test.ts` has no I/O beyond reading a file committed to the repository; `*.int.test.ts` touches something real: a child process, a temporary directory or git repository, or a network socket the test itself listens on. Page scripts are tested under jsdom. Tests assert behaviour, not implementation. No skipped or focused tests. No test is deleted or narrowed while the behaviour it covered remains: for every deleted or shrunk test file, check whether the code it exercised still exists at head. A phase that touches ask, poll, mode or agentic behaviour records in the PR draft's *Verification* either a Pro battery run or the line `Pro battery: pending, the owner runs it`. Pending is not a finding at review: the run spends the owner's Pro queries, and `/review-phase` §8 has it run and recorded before the PR opens. A draft with neither on such a phase is a finding.
