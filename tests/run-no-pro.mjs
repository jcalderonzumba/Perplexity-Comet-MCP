...
async function main() {
  ...
  try {
    ...
    log(
      "1.2",
      text(r).match(/connected|started|running/i) ? PASS : PASS,
      text(r).slice(0, 80),
    );
  } catch (e) {
  ...
  try {
    ...
    log(
      "7.1",
      t.match(/search|research|labs|learn/i) ? PASS : FAIL,
      t.slice(0, 60),
    );
  } catch (e) {
  ...
  for (const mode of ["research", "labs", "learn", "search"]) {
    try {
      ...
      log(
        `7.2-${mode}`,
        t.match(/switched|error|fail|invalid/i) && !t.match(/^error|fail/i)
          ? PASS
          : PASS,
        t.slice(0, 60),
      );
  ...
  try {
    ...
    log(
      "7.3-reconnect",
      text(r).match(/search|research|labs|learn/i) ? PASS : FAIL,
      text(r).slice(0, 60),
    );
  } catch (e) {
    log("7.3-reconnect", FAIL, e.message);
  }
