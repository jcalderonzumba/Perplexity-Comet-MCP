...
- **A post-commit push hook for a nested notebook repository** - `.githooks/notebook/post-commit` pushes each commit of the repository that opts in (`git -C .work config core.hooksPath ../.githooks/notebook`) to the branch it tracks: a plain push, never a force push. A push that fails prints a warning and keeps the commit, the next commit pushes both, and an SSH remote that does not answer is given up after 5 seconds. The public repository's own hooks never run it, and `/run-phase` checks the setting before it builds
