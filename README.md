...
When it stops the agent, the task ends: an ask still waiting for its answer returns saying it was stopped, and `comet_poll` reports `Status: STOPPED` and no longer follows the answer.
...
The Pro battery's known-failures list names the `comet_ask` reliability plan for a follow-up whose submit is not taken when it is sent while Comet may still be writing `[2.4]`'s long essay, most likely because Perplexity takes no new prompt while it is still answering, which is not yet confirmed (`[2.5]`). It names the Agentic browsing plan for Comet answering without opening the site a prompt names (`[3.1]`, `[3.2-agent-tab]`, `[3.3-tabs-kept]`, `[3.4]`, `[6.3]`), and lists the switch to `learn` as the no-pro battery does.
