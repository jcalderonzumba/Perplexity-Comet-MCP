...
- **Claude Code hooks** - `.claude/settings.json` runs `protect-main.sh` on git commands and `pr-gate.sh` on `gh pr create` and `gh pr merge`, refusing writes to `main` and pull requests whose last commit lacks either stamp. `protect-main.sh` exempts git commands aimed at the nested notebook (`git -C .work …`), and judges a command's text twice, as written and with its quoting read as the shell reads it (double, single and `$'…'` quotes, backslash escapes, newlines inside quotes), refusing when either reading could still reach `main`: a global option's argument quoted or escaped, spaces and all, a line continued with a backslash, `main` quoted, escaped or beside a parenthesis in a push, and a notebook command after any `GIT_` variable the command names, such as `GIT_DIR`, `GIT_WORK_TREE` or `GIT_CONFIG_*`
